...
bool drawTextureFragment(sf::RenderWindow& window, const sf::Texture& texture, const sf::FloatRect& dst, sf::IntRect src)
{
    ...
    src.left = std::clamp(src.left, 0, static_cast<int>(size.x) - 1);
    src.top = std::clamp(src.top, 0, static_cast<int>(size.y) - 1);
...
bool drawLevelTileFragment(sf::RenderWindow& window, const AssetManager& assets, int levelNumber, const sf::FloatRect& dst, sf::IntRect src)
{
...
{
    return tile == 'G' || tile == 'D' || tile == 'B';
}

bool isOpenForTerrain(char tile)
{
...
{
    const sf::Texture* texture = assets.texture(levelVisualAssets(levelNumber).background);
    if (!texture || texture->getSize().x == 0 || texture->getSize().y == 0)
        return false;
...
bool Level::saveCustom(const std::filesystem::path& path) const
{
    ...
    file << "{\n";
    file << "  \"type\": \"customLevel\",\n";
    file << "  \"columns\": " << m_columns << ",\n";
    file << "  \"tiles\": [\n";
    for (std::size_t row = 0; row < m_tiles.size(); ++row) {
        file << "    \"" << m_tiles[row] << "\"";
        if (row + 1 < m_tiles.size())
            file << ",";
        file << "\n";
    }
    file << "  ]\n";
...
bool Level::loadCustom(const std::filesystem::path& path)
{
    ...
    const std::size_t key = json.find("\"tiles\"");
    ...
    const std::string body = json.substr(begin + 1, end - begin - 1);
    const std::regex rowRegex("\"([^\"]*)\"");
    for (auto it = std::sregex_iterator(body.begin(), body.end(), rowRegex); it != std::sregex_iterator(); ++it)
        rows.push_back((*it)[1].str());

    if (rows.size() != Rows || rows.front().empty())
        return false;
    ...
    clear(static_cast<int>(columns));
    m_tiles = std::move(rows);
    m_number = 1;
    m_start = {3.0f * Tile, 9.0f * Tile};
    m_flag = {std::max(5, m_columns - 6) * Tile, 6.0f * Tile};
    m_skyTop = sf::Color(92, 184, 232);
    m_skyBottom = sf::Color(184, 224, 248);
    m_headline = "Custom level ukonczony.";

    for (int row = 0; row < Rows; ++row) {
        for (int col = 0; col < m_columns; ++col) {
            if (tileAt(row, col) == 'F') {
                m_flag = {col * Tile, row * Tile};
                return true;
            }
        }
    }
...
void Level::draw(sf::RenderWindow& window, const AssetManager& assets, WorldMode world, bool secretsRevealed, float time, bool castleInterior) const
{
    ...
    for (int row = 0; row < Rows; ++row) {
        ...
        while (col <= lastCol) {
            ...
            while (col <= lastCol && tileAt(row, col) == 'P')
                ++col;
            const int endCol = col - 1;

            int bottomRow = row;
            for (int y = row + 1; y < Rows; ++y) {
                bool fullPipeRow = true;
            ...
            const sf::FloatRect pipeBounds(startCol * Tile, row * Tile, (endCol - startCol + 1) * Tile, (bottomRow - row + 1) * Tile);
            if (!drawLevelPipe(window, assets, m_number, pipeBounds)) {
                for (int y = row; y <= bottomRow; ++y) {
                    for (int x = startCol; x <= endCol; ++x)
                        drawTile(window, assets, y == row ? 'P' : 'p', tileRect(y, x), world, secretsRevealed, time);
                }
            }
        }
    }
    ...
    const int visualLastRow = static_cast<int>(std::ceil(viewBottom / Tile));
    for (int col = firstCol; col <= lastCol; ++col) {
        int fillStartRow = -1;
        for (int row = Rows - 1; row >= GroundRow; --row) {
            const char tile = tileAt(row, col);
            if (tile == '~')
                break;
            if (tile == 'G' || tile == 'D') {
...
void Level::drawTile(sf::RenderWindow& window, const AssetManager& assets, char tile, const sf::FloatRect& rect, WorldMode world, bool secretsRevealed, float time) const
{
    ...
    if (tile == 'Z') {
        ...
        if (drawLevelTerrainTile(window, assets, m_number, rect, TerrainSprite::Crack))
            return;
